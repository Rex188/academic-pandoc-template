...
Empirical experiments on our custom gesture-augmented instrument, Robin, quickly overturned this initial hypothesis. In rapid percussive rolls driven by high-frequency wrist oscillations ($8\sim 12\text{ Hz}$), the linear Fourier slope collapsed to $\beta \approx 0.16$, while lag-1 autocorrelation plummeted. Judged by a single linear metric, the performer's high-speed virtuosity looked indistinguishable from uncorrelated white noise. Simultaneously, testing against synthetic benchmarks demonstrated that deterministic nonlinear systems produce fractured sub-band slopes ($\Delta_\beta = 2.54$), rendering a single regression slope mathematically ill-defined.

These empirical anomalies exposed a fundamental blind spot in linear spectral analysis: *linear metrics cannot distinguish low-dimensional deterministic dynamics from linearly filtered Gaussian noise.* Any Gaussian white noise passed through an appropriate linear filter precisely reproduces an empirical $1/f$ spectrum without possessing underlying state-dependent constraints [@taqqu1995]. Observing a $1/f$ slope cannot determine whether bodily variability reflects an unconstrained random walk or a deterministic neuromuscular control manifold.
...
Certified against synthetic controls (PRNG noise and chaotic maps) and hardware decimation audits, our framework resolves the rapid roll paradox. Where linear metrics misclassify high-speed rolls as white noise, Axis 2 unmasks their rigorous deterministic coordination ($p = 0.0198$).

Our findings advance gestural interaction along three dimensions. First, we provide experimental proof that the single-index spectral exponent $\beta$ collapses under high-frequency entrainment and deterministic chaos, mistaking rapid rolls for uncorrelated noise ($\beta \approx 0.16$). Second, across all physical axes and performance regimes, Robin accelerometer streams decisively reject the linear surrogate null ($p < 0.02$). Bodily gestures do not drift through unconstrained random walks; they evolve along deterministic nonlinear manifolds. Third, we release a fully verified experimental pipeline and sensor benchmark for single-case gestural dynamics, anchoring gesture-aware synthesis in physical mechanics rather than speculative metaphysical claims.
...
For $B = 100$, the minimum achievable rank floor is $p_{\text{min}} = 1/101 \approx 0.0099$ (or $2/101 \approx 0.0198$ for a two-sided test). We adopt a significance threshold of $\alpha = 0.05$. When $p < 0.05$, the linear Gaussian null hypothesis is rejected in favor of low-dimensional deterministic structure.
...
We evaluated the primary three-axis accelerometer streams logged from Robin across the three percussive performance regimes. As shown in @tbl-empirical-gestures, Axis 1 spectral characteristics diverge across physical regimes, whereas Axis 2 identifies invariant deterministic structure across all axes and conditions.
...
3. **Regime T3 (Rapid Percussive Rolls)**: Breaks the colored noise pattern entirely: the apparent spectral slope collapses to $\beta = 0.16 \pm 0.12$ ($X$-axis) and $\beta = 0.21 \pm 0.11$ ($Y$-axis). Voluntary rapid wrist articulation concentrated in the $3\sim 6\text{ Hz}$ sub-band injects kinetic energy into the upper observable spectrum (strictly within the $8.33\text{ Hz}$ Nyquist bandwidth of native $16.67\text{ Hz}$ transmission), flattening the global spectral slope.
...
- Even when constrained to identical Fourier power spectra and marginal histograms, linear stochastic processes fail to replicate the low entropy of human motor variability. Bodily fluctuations during expressive performance reflect structured, phase-coupled nonlinear temporal regularity rather than linear stochastic drift.
...
2. **Existential Falsification and Construct Validity**: Nonetheless, an idiographic $N=1$ design possesses decisive epistemological power. Just as observing a single black swan falsifies the assertion that all swans are white, observing the catastrophic collapse of the spectral exponent in Trial 3 ($\beta \approx 0.16$) decisively falsifies the long-standing dogma that expressive human motor variability universally conforms to $1/f$ pink noise. Moreover, the Two-Axis Framework demonstrates high diagnostic construct validity, successfully decoupling macro-gesture classification from micro-structural nonlinear regularity across all spatial axes.
3. **Future Work Roadmap**: This study establishes clear empirical and theoretical signposts for subsequent investigation:
   - *Multi-Performer Cohorts*: Conducting systematic cohort studies comparing percussionists, string players, and non-musicians to determine how formal motor training modulates phase-space orbital stability.
   - *Cross-Interface Benchmarking*: Applying the Two-Axis diagnostic pipeline across distinct interaction paradigms (continuous ribbons, multidimensional touch surfaces, and camera-based vision systems).
   - *Ultra-High-Speed Multimodal Capture*: Synchronizing 240 Hz optical motion capture with 1000 Hz surface electromyography (sEMG) to evaluate formal attractor dimensions ($D_2$) and Lyapunov spectra under long-duration stationary conditions.
   - *Embedded Real-Time Firmware*: Implementing prototype nuance-preserving nonlinear filters inside digital synthesizer firmware to evaluate latency, tactile transparency, and performer agency through double-blind evaluation.
